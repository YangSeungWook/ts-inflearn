...
textLength(10); // 숫자에는 length api가 없기땨문에 오류
textLength('good'); // 문자열 가능
textLength({length:10}); // 인터페이스에 타입 선언되어 있어서 가능
...
//keyof로 사용시 하나의 key값으로만 사용이 가능
function getShoppingItemOption<T extends keyof ShoppingItem>(itemOption:T) : T {
    return itemOption;
}
// getShoppingItemOption(10); // 불가
// getShoppingItemOption<string>('a'); // 불가
